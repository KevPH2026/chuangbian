...
export const CATEGORY_ORDER = ["all", "gif", "question", "money", "work", "love", "ai", "default", "opossum-original", "recovery"];
...
export async function saveGeneratedGif({
  gif,
  model,
  meta,
  prompt,
  ...
  const combo = inputCombo || buildCombo({ text: meta?.caption, role: meta?.role, variant: "gif" });
  const existing = await findGalleryItem(combo.comboKey);
  if (existing) {
    return existing;
  }

  const gifBlob = await uploadGeneratedAsset(combo.comboKey, gif, { preserveOriginal: true });
  const now = new Date().toISOString();
  const item = {
    id: combo.comboKey,
    comboKey: combo.comboKey,
    ...
    role: combo.role,
    roleName: meta?.roleName || combo.roleName,
    action: meta?.action || "gif",
    actionName: meta?.actionName || "动作 GIF",
    category: "gif",
    categoryName: getCategoryName("gif"),
    creatorId,
    creatorName: creatorName || "无名受害者",
    creatorEmailHash,
    model,
    imageUrl: gifBlob.url,
    downloadUrl: gifBlob.downloadUrl || gifBlob.url,
    thumbnail: gifBlob.url,
    mediaType: "gif",
    contentType: gifBlob.contentType || "image/gif",
    promptVersion: prompt ? "2026-05-30-action-gif" : "2026-05-30-gif",
    createdAt: now,
    lastUsedAt: now,
    uses: 1
  };
...
async function uploadGeneratedImage(comboKey, image) {
  return uploadGeneratedAsset(comboKey, image);
}

async function uploadGeneratedAsset(comboKey, image, { preserveOriginal = false } = {}) {
  if (!hasBlobToken()) {
    return { url: image, downloadUrl: image };
  }

  const { buffer, contentType, extension } = preserveOriginal ? await imageToOriginalBuffer(image) : await imageToBuffer(image);
...
async function imageToBuffer(image) {
  const { buffer, contentType } = await imageSourceToRawBuffer(image);
  return compressForMeme(buffer, contentType);
}

async function imageToOriginalBuffer(image) {
  const { buffer, contentType } = await imageSourceToRawBuffer(image);
  return {
    buffer,
    contentType,
...
export async function splitImageGridToDataUrls(image, columns = 3, rows = 3) {
  const { buffer } = await imageSourceToRawBuffer(image);
  const source = sharp(buffer, { animated: false });
  const metadata = await source.metadata();
  const width = Number(metadata.width || 0);
  const height = Number(metadata.height || 0);
  if (!width || !height || width < columns || height < rows) {
    throw new Error("职场包总图尺寸异常，没法切成 9 张。");
  }
  ...
  for (let row = 0; row < rows; row += 1) {
    for (let column = 0; column < columns; column += 1) {
      ...
      const tile = await sharp(buffer, { animated: false })
        ...
        .toBuffer();
      tiles.push(`data:image/png;base64,${tile.toString("base64")}`);
    }
  }
  return tiles;
}

async function imageSourceToRawBuffer(image) {
...
function extensionFromContentType(contentType) {
  ...
  if (cleanType.includes("gif")) {
    return "gif";
  }
  if (cleanType.includes("webp")) {
    return "webp";
  }
  if (cleanType.includes("jpeg") || cleanType.includes("jpg")) {
    return "jpg";
  }
  return "png";
}
