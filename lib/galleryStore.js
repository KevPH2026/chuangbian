...
async function uploadGeneratedImage(comboKey, image) {
  if (isGithubStorageEnabled()) {
    ...
    return putGithubFile(`${IMAGE_PREFIX}/${comboKey}.${extension}`, buffer, { contentType });
  }
...
async function readIndex() {
  if (isGithubStorageEnabled()) {
    return normalizeIndex((await readGithubJson(INDEX_PATH)) || { items: [] });
  }
...
async function writeIndex(index) {
  ...
  if (isGithubStorageEnabled()) {
    await writeGithubJson(INDEX_PATH, nextIndex);
    return nextIndex;
  }
