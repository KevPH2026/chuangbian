...
export async function sendLoginCode({ email = "", name = "", ip = "", userAgent = "" }) {
  ...
  const codeHash = signValue(`${cleanEmail.toLowerCase()}:${code}`);
  const expiresAt = now + CODE_TTL_MS;
  const loginTicket = signLoginTicket({
    codeHash,
  ...
  await writeAuthIndex(index).catch(() => null);
...
export async function verifyLoginCode({
  ...
  const ticketRecord = verifyLoginTicket(loginTicket, cleanEmail);
  if (ticketRecord) {
    if (Number(ticketRecord.expiresAt || 0) < Date.now()) {
    ...
    if (!safeEqual(String(ticketRecord.codeHash || ""), actual)) {
      const error = new Error("验证码不对。窗边没有这么好骗。");
      error.status = 400;
      throw error;
    }

    await forgetStoredCode(cleanEmail).catch(() => null);
...
async function completeLogin({ cleanEmail, cleanName, ip = "", referralCode = "", userAgent = "", viewerId = "" }) {
...
function signLoginTicket(payload) {
  const payloadPart = Buffer.from(JSON.stringify(payload)).toString("base64url");
  return `${payloadPart}.${signValue(payloadPart)}`;
}

function verifyLoginTicket(ticket, email) {
  const raw = String(ticket || "").trim();
  const [payloadPart, signature] = raw.split(".");
  if (!payloadPart || !signature || !safeEqual(signValue(payloadPart), signature)) {
    return null;
  }

  try {
    const payload = JSON.parse(Buffer.from(payloadPart, "base64url").toString("utf8"));
    if (payload?.email !== String(email || "").trim().toLowerCase() || !payload?.codeHash) {
      return null;
    }
    return payload;
  } catch {
...
async function forgetStoredCode(email) {
  const index = await readAuthIndex();
  const emailHash = hashValue(String(email || "").trim().toLowerCase());
  if (index.codes[emailHash]) {
    delete index.codes[emailHash];
    await writeAuthIndex(index);
  }
}
