...
async function readModelConfig() {
  if (isGithubStorageEnabled()) {
    return readGithubJson(MODEL_CONFIG_PATH);
  }
  ...
  try {
    ...
    return readBlobJson(metadata.url, { token: process.env.BLOB_READ_WRITE_TOKEN });
  } catch (error) {
...
async function writeModelConfig(config) {
  ...
  if (isGithubStorageEnabled()) {
    await writeGithubJson(MODEL_CONFIG_PATH, normalized);
    return normalized;
  }
