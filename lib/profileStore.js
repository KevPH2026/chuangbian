...
export async function saveUserProfileUpload({ avatarImage = "", ip = "", referralCode = "", userAgent = "", userEmail = "", userName = "", viewerId = "" }) {
  const cleanAvatar = avatarImage ? cleanAvatarImage(avatarImage) : "";
  const email = cleanCreatorEmail(userEmail);
  const name = cleanCreatorName(userName);
  if (!email || name === "无名受害者") {
    const error = new Error("先填昵称和邮箱，别糊弄登录。");
    error.status = 400;
    throw error;
  }
  ...
  const avatarHash = cleanAvatar ? hashValue(cleanAvatar).slice(0, 20) : existing?.avatarHash || "";
  const previousHash = existing?.avatarHash || "";
  const avatarBlob = cleanAvatar ? await uploadProfileImage(profileId, avatarHash, cleanAvatar) : { url: existing?.avatarUrl || "" };
  const nextItem = {
    ...(existing || {}),
    id: profileId,
    avatarHash,
    ...
    updatedAt: now,
    uploadCount: Number(existing?.uploadCount || 0) + (cleanAvatar && previousHash !== avatarHash ? 1 : 0),
...
function normalizeProfileIndex(index) {
  return {
    items: Array.isArray(index?.items) ? index.items.filter((item) => item?.id).map(normalizeProfile) : [],
