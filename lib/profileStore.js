...
import { isGithubStorageEnabled, putGithubFile, readGithubJson, writeGithubJson } from "./githubStorage.js";
...
async function uploadProfileImage(profileId, avatarHash, avatarImage) {
  if (isGithubStorageEnabled()) {
    ...
    return putGithubFile(`${PROFILE_IMAGE_PREFIX}/${hashValue(profileId).slice(0, 20)}-${avatarHash}.${extension}`, buffer, { contentType });
  }
...
async function readProfileIndex() {
  if (isGithubStorageEnabled()) {
    return normalizeProfileIndex((await readGithubJson(PROFILE_PATH)) || memoryProfiles);
  }
  ...
  try {
    ...
    return normalizeProfileIndex(await readBlobJson(metadata.url, { token: process.env.BLOB_READ_WRITE_TOKEN }));
  } catch (error) {
...
async function writeProfileIndex(index) {
  ...
  if (isGithubStorageEnabled()) {
    await writeGithubJson(PROFILE_PATH, normalized);
    return normalized;
  }
