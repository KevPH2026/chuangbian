...
export async function createActionGif({ body = {}, env = process.env, req }) {
  ...
  const rolePreset = getRolePreset(body.role);
  ...
  const gifVariant = `gif:${action.id}:${avatarImage ? hashGalleryVariant(avatarImage) : ""}`;
  const combo = buildCombo({ text, role: rolePreset.id, variant: gifVariant });
  ...
  const prompt = buildActionGifPrompt({ action, hasAvatar: Boolean(avatarImage), rolePreset, text });
  const sheetImage = await generateImage({
    ...
    referenceImage: wantsAvatar ? avatarImage : "",
