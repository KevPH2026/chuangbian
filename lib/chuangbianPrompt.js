export const ROLE_PRESETS = [
  {
    ...
    style:
      "strictly preserve the original opossum window meme style: photographic, slightly blurry, low-resolution repost screenshot feel, blue night window, gray frame and handle, simple crop, no redesign, no 3D, no Pixar, no cute plush toy"
  },
  {
    id: "glasses-man",
    name: "Q版眼镜男",
...
const ACTION_PRESETS = [
  ...
  },
  {
    id: "interaction",
    name: "互动回应",
    test: isInteractionLike,
    pose:
      "the character is still standing at the blue window, but reacts to someone in the room: feet and lower body remain near the window, upper body and face turn toward the viewer, one hand or paw reaches out as if greeting, shaking hands, saying hello, receiving something, or responding politely; keep the other hand close to the back when possible",
    expression: "awkward polite social interaction, slightly confused but friendly, meme-like",
    textTreatment:
      "large bold white Chinese caption over the image, with a soft black drop shadow, no fancy layout"
  },
  {
    id: "money",
    name: "缩脖低气压",
...
function isQuestionLike(text) {
  return /[?？]|^啊[？?]?$|吗$|呢$|么$|啥|什么|为啥|为什么|怎么|咋|谁|哪|凭什么|合理吗|真的假的|尊嘟假嘟|不是哥们/i.test(
    String(text || "").trim()
  );
}

function isInteractionLike(text) {
  return /你好|您好|哈喽|hello|hi|早上好|晚上好|在吗|来啦|来了|辛苦|谢谢|感谢|拜拜|再见|握手|交个朋友|抱一下|给你|拿去|欢迎|请坐|请问|收到|OK|ok|好的|行吧|可以|不客气|麻烦你|帮我|你先|你来|你说|老板好|老师好|同学好/i.test(
    String(text || "").trim()
  );
}
...
export function buildChuangbianPrompt({ text, role, hasAvatar = false, userName = "" }) {
  ...
  if (rolePreset.id === "opossum" && !["question", "interaction"].includes(action.id)) {
    action = {
      id: "opossum-original",
      name: "原图侧影",
      pose:
  ...
  const roleLine =
    rolePreset.id === "opossum"
      ? "the original viral window opossum look: gray and white real opossum, narrow snout, small black ears, photographic low-resolution repost screenshot, slightly blurry, compressed, not cute, not redesigned, no 3D"
  ...
  const actionLine =
    action.id === "question"
      ...
      : action.id === "interaction"
        ? "interpret the caption as social interaction: the character stays by the window but turns face and upper body toward the viewer, one hand or paw reaches forward in a greeting or handshake gesture, still keeping the blue-window meme composition"
  ...
  const prompt = [
    "1:1 square Chinese internet meme image, final canvas must be square, not vertical poster, not 4:5",
    roleLine,
    "first infer whether the caption is social interaction or private reflection: interaction captions should make the character face the viewer and gesture; reflective captions should keep the character facing the window",
