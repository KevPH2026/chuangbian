...
async function readQuotaIndex() {
  if (isGithubStorageEnabled()) {
    return normalizeIndex((await readGithubJson(QUOTA_PATH)) || createEmptyIndex());
  }
  ...
  try {
    ...
    return normalizeIndex(await readBlobJson(metadata.url, { token: process.env.BLOB_READ_WRITE_TOKEN }));
  } catch (error) {
...
async function writeQuotaIndex(index) {
  ...
  if (isGithubStorageEnabled()) {
    await writeGithubJson(QUOTA_PATH, normalized);
    return;
  }
